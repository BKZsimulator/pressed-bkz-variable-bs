...
public:
  ...
  */
  ...
  BKZParam(int block_size, vector<Strategy> &strategies, double delta = LLL_DEF_DELTA,
           int flags = BKZ_DEFAULT, int max_loops = 0, int iter_tours = 0, double max_time = 0,
      ...
      : block_size(block_size), strategies(strategies), delta(delta), flags(flags),
        max_loops(max_loops), iter_tours(iter_tours), max_time(max_time), auto_abort_scale(auto_abort_scale),
  ...
  /** number of tours for each iteration in presse-bkz **/
  int iter_tours;  
